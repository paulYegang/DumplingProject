...
- (void)myInit {
    ...
    [self.buttons setCenter:CGPointMake(kScreenBounds.size.width / 2, kScreenBounds.size.height - 100)];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == self.images.count - 1) {
        [cell.buttons setHidden:NO];
        [cell.buttons addTarget:self action:@selector(nextButtonHandler:) forControlEvents:UIControlEventTouchUpInside];
    } else {
        [cell.buttons setHidden:YES];
    }
